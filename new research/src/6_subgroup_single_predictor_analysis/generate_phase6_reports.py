...
def prep(res):
    ...
    r["Predictor"] = r["predictor"].map(lambda p: PREDICTORS[p][0])
    r["_po"] = r["predictor"].map({p: i for i, p in enumerate(PREDICTORS)})
    r["_oo"] = r["outcome"].map({c: i for i, (_, c, *_) in enumerate(OUTCOMES)})
    return r


COLS = ["Predictor", "n", "Effect per 1 SD (95% CI)", "raw slope", "t / z", "p (raw)", "q (BH, all tests in population)", "q (BH, within outcome)", "dAIC vs covariates", "dAIC vs HbA1c model", "CV R2/AUC (pred | covs only)"]
HDR = ["Predictor (entered alone)", "n", "Effect per 1 SD (95% CI)", "Raw slope", "t / z", "p (raw)", "q (BH, family = all tests in population)", "q (BH, family = this outcome)", "dAIC vs covariates-only", "dAIC vs HbA1c-only", "CV R2 / AUC (predictor | covariates only)"]
...
def write_bands(res, fe, cfg):
    ...
    for band in ["70-180", "54-69", "<70", "54-250", ">180", "181-250", ">250", "<54"]:
        ...
        for g, (glabel, _) in GROUPS.items():
            ...
            okk = okk.copy(); okk["Outcome"] = okk["label"]
            okk["exposed"] = okk["n_exposed_gt0"].map(lambda v: "-" if pd.isna(v) else f"{int(v)}")
            L.append(md_table(okk, ["Outcome"] + COLS[:2] + ["exposed"] + COLS[2:], ["Outcome"] + HDR[:2] + ["n with any time in band"] + HDR[2:]))
            sk = sub[sub["skipped_reason"].fillna("") != ""]
            if len(sk):
