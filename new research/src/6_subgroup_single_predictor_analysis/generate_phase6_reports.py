...
def md_table(df, cols, headers=None):
    ...
    out = ["| " + " | ".join(headers) + " |", "| " + " | ".join([":---"] + [":---:"] * (len(cols) - 1)) + " |"]
...
HDR_B = ["Predictor (alone)", "β per 1 SD (95% CI)", "q, all tests in population", "q, this outcome", "Adj R² / AUC", "ΔAIC vs covs", "ΔAIC vs HbA1c", "CV R²/AUC (pred | covs)"]
