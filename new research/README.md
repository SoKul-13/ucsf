...
# 27. Full term-by-term model output tables for every Phase 6 / 6b model (standard OLS-output format)
python3 "new research/src/6_subgroup_single_predictor_analysis/export_model_output_tables.py"
...
| **Phase 6 / 6b** | `reports/6_subgroup_single_predictor_analysis/model_output_tables/` (index `README.md`; `phase6/<population>/<domain>.md`, `phase6b/<cohort>/<population>/<domain>.md`, `phase6_model_outputs.csv`, `phase6b_model_outputs.csv`) | `src/6_subgroup_single_predictor_analysis/export_model_output_tables.py` | Every fitted model with all terms (intercept, covariates, glycaemic predictors) in the standard OLS-output table format: formula, N, R²/adj R² or pseudo-R²/AUC, F or LLR test, RSE, AIC, BIC; coef, SE, ±2SE, t/z, p, signif. |
