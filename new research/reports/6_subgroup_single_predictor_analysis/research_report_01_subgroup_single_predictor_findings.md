...
**Full model outputs**: every single-predictor model with all terms in the standard OLS-output table format is in `model_output_tables/phase6_<population>.md` and `phase6_model_outputs.csv` (Phase 6b cohorts: `phase6b_<cohort>_<population>.md`, `phase6b_model_outputs.csv`), generated by `src/6_subgroup_single_predictor_analysis/export_model_output_tables.py`.
