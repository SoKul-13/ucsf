...
**Scope.** 92 single-predictor tests; 4 with raw p < 0.05 (about 5 expected by chance); FDR rule applied to 92 tests (samples with n >= 500), of which **0** are significant at BH q < 0.05 in the all-tests family and 0 in the within-outcome family.

**By outcome (best out-of-sample predictor, then the associations that survive FDR):**

- **Indoor PM2.5, log(1 + mean ug/m3)** (n = 674): best single predictor out of sample is **Mean glucose** (CV R² 0.097 vs 0.098 for covariates alone, gain -0.000; -0.0351 per SD, p = 0.273, q = 0.739). No glycaemic measure is associated with this outcome (all p > 0.05). Not predictable from glycaemia in this sample.
- **Indoor temperature, mean (deg C)** (n = 674): best single predictor out of sample is **GMI** (CV R² 0.310 vs 0.311 for covariates alone, gain -0.001; +0.0361 per SD, p = 0.616, q = 0.918). No glycaemic measure is associated with this outcome (all p > 0.05). Not predictable from glycaemia in this sample.
- **Indoor relative humidity, mean (%)** (n = 674): best single predictor out of sample is **CV** (CV R² 0.243 vs 0.241 for covariates alone, gain +0.002; +0.397 per SD, p = 0.079, q = 0.554). No glycaemic measure is associated with this outcome (all p > 0.05).
- **Indoor VOC index, mean** (n = 674): best single predictor out of sample is **SD (daily avg)** (CV R² -0.005 vs -0.015 for covariates alone, gain +0.009; -1.54 per SD, p = 0.013, q = 0.254). No association survives FDR; nominal only: SD (daily avg) (p = 0.013), SD (pooled) (p = 0.016), GMI (p = 0.024), Mean glucose (p = 0.024).

**Most predictable outcomes (largest out-of-sample gain over covariates):** Indoor VOC index, mean (+0.009, via SD (daily avg)); Indoor relative humidity, mean (%) (+0.002, via CV); Indoor PM2.5, log(1 + mean ug/m3) (-0.000, via Mean glucose); Indoor temperature, mean (deg C) (-0.001, via GMI). Gains below about 0.01 are negligible for individual-level prediction even when statistically significant.

**Predictor families carrying the signal:** CGM level (0 FDR-significant / 2 raw-significant of 12); CGM variability (0 FDR-significant / 2 raw-significant of 32); HbA1c (0 FDR-significant / 0 raw-significant of 4).
Level metrics: 0 FDR-significant (2 raw); variability metrics: 0 FDR-significant (2 raw); HbA1c alone: 0 FDR-significant (0 raw) across the outcomes in this file.

**Where a CGM metric beats HbA1c by more than 2 AIC on the same rows:** Indoor relative humidity, mean (SD (daily avg), ΔAIC -3.2); Indoor VOC index, mean (SD (daily avg), ΔAIC -6.1).
